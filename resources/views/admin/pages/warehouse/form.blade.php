...
<h4 class="mb-4 text-lg font-semibold text-gray-600 dark:text-gray-300">
    @if (isset($warehouse)) Update @else Add @endif Warehouse
</h4>
<form action="{{ isset($warehouse) ? route('warehouse.update', $warehouse->id) : route('warehouse.store') }}" method="post" class="mt-5">
    @csrf
    <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
        <label class="block text-sm">
            <span class="text-gray-700 dark:text-gray-400">Name</span>
            <x-input placeholder="Enter warehouse Name" name="name" type="text" value="{{ old('name', $warehouse->name ?? '') }}" required />
            @if($errors->has('name'))
